	...
	<section class="Technology LayoutTwo-vertical" :style="{'--delai': technology.bulletsDelay + 's'}">
		...
		<div class="Technology-media LayoutTwo-media LayoutTwo-size70 u-relative Technology-image u-mediaZoomAnimation">
			<div class="LayoutTwo-mobileMask LayoutTwo-mobileMask--fromLeft">
				<img :src="technology.image" :alt="technology.title" class="u-mediaFullSize">
			</div>
		</div>

		<div class="LayoutTwo-text LayoutTwo-size30 Technology-text">

			<div class="Technology-timer" v-if="technology.bullets">
				<ul class="Technology-timerList">
					<li class="Technology-timerListItem Paragraph" v-for="(bullet, key) in bullets" :key="bullet">
						<transition><span class="timer" v-show="counter == key + 1">{{ bullet }}</span></transition>
					</li>
				</ul>
				<div class="Technology-timerLines" ref="techLines">
					<div v-for="(bullet, key) in bullets" :key="bullet" class="Technology-timerLinesItem" :class="{'is-active': (counter == key + 1) , 'is-finished': (counter >= key + 2)}"></div>
				</div>
			</div>
